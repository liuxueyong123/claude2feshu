...
test("registerSession creates a new session", async () => {
  ...
  try {
    const { registerSession, getSession } = await import(`../src/session/state.ts?case=new-${Date.now()}`);
...
test("getSession returns null for unknown session", async () => {
  ...
  try {
    const { getSession } = await import(`../src/session/state.ts?case=unknown-${Date.now()}`);
...
test("findByPrefix matches session_id by prefix (>= 8 chars)", async () => {
  ...
  try {
    const { registerSession, findByPrefix } = await import(`../src/session/state.ts?case=prefix-${Date.now()}`);
...
test("findByPrefix returns null for prefix shorter than 8 characters", async () => {
  ...
  try {
    const { registerSession, findByPrefix } = await import(`../src/session/state.ts?case=short-${Date.now()}`);
    registerSession(makeSessionState({ session_id: "abc123-def456-ghi789" }));
...
test("isProcessAlive returns true for running process", async () => {
  const { isProcessAlive } = await import("../src/session/state.js");
...
test("listActive filters out sessions with dead processes", async () => {
  ...
  try {
    const { registerSession, listActive } = await import(`../src/session/state.ts?case=listactive-${Date.now()}`);
...
test("markIdle sets session status to idle", async () => {
  ...
  try {
    const { registerSession, markIdle, getSession } = await import(`../src/session/state.ts?case=markidle-${Date.now()}`);
...
test("updateHeartbeat refreshes last_heartbeat to near-now", async () => {
  ...
  try {
    const { registerSession, updateHeartbeat, getSession } = await import(`../src/session/state.ts?case=heartbeat-${Date.now()}`);
...
test("removeSession deletes session record", async () => {
  ...
  try {
    const { registerSession, removeSession, getSession } = await import(`../src/session/state.ts?case=remove-${Date.now()}`);
...
test("registerSession marks old sessions with same PID as idle", async () => {
  ...
  try {
    const { registerSession, getSession } = await import(`../src/session/state.ts?case=pid-dedup-${Date.now()}`);
