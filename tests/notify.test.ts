...
test("checkInboxText says empty when no pending inbox items", async () => {
  ...
  try {
    const { checkInboxText } = await import(`../src/notify/index.ts?case=empty-${Date.now()}`);
...
test("checkInboxText lists pending inbox messages", async () => {
  ...
  try {
    const mq = await import(`../src/session/queue.ts?case=notify-inbox-${Date.now()}`);
    mq.enqueue({
      id: "om_test_1",
      chat_id: "oc_1",
      sender: "ou_user",
    ...
    const { checkInboxText } = await import(`../src/notify/index.ts?case=inbox-${Date.now()}`);
...
test("popInboxCommand says empty when no pending items", async () => {
  ...
  try {
    const { popInboxCommand } = await import(`../src/notify/index.ts?case=pop-empty-${Date.now()}`);
...
test("popInboxCommand returns first pending inbox command", async () => {
  ...
  try {
    const mq = await import(`../src/session/queue.ts?case=notify-pop-${Date.now()}`);
    mq.enqueue({
      id: "om_test_2",
      chat_id: "oc_1",
      sender: "ou_user2",
    ...
    const { popInboxCommand } = await import(`../src/notify/index.ts?case=pop-${Date.now()}`);
